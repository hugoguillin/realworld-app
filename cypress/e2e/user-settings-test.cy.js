...
describe('user-settings-test', () => {
  ...
  beforeEach(() => {
    ...
    CommonApi.registerNewUser(newUserData).as('newUser').then(newUser => {
  ...
  it('updates url of profile picture', () => {
    ...
    //Act
    UserSettingsPage.updateField(UserSettingsFields.image, fieldsToUpdate.user.image)

    //Assert
    UserSettingsPage.userPic().invoke('attr', 'src').should('eq', fieldsToUpdate.user.image)
  })
  ...
  it('updates username', () => {
    ...
    //Assert
      expect(user.response.body.user.username, 'User username').to.eq(fieldsToUpdate.user.username)
    })
    UserSettingsPage.userPic().parent().should('have.text', fieldsToUpdate.user.username)
  })
  ...
  it('updates user bio', () => {
    ...
    //Act
    UserSettingsPage.updateField(UserSettingsFields.bio, fieldsToUpdate.user.bio)
    cy.wait('@updateUser').then(user => {

    //Assert
      expect(user.response.body.user.bio, 'User bio').to.eq(fieldsToUpdate.user.bio)
    })
    UserSettingsPage.getField(UserSettingsFields.bio).should('have.value', fieldsToUpdate.user.bio)
  })
  ...
  it('updates email', () => {
    ...
    UserSettingsPage.getField(UserSettingsFields.email).should('have.value', userNewEmail)
  })
  ...
  it('updates password', () => {
    ...
    UserSettingsPage.getField(UserSettingsFields.password).invoke('val').should('eq', newPassword)
  })
  ...
  it('updates all fields at once', () => {
    ...
    //Assert
    UserSettingsPage.userPic().invoke('attr', 'src').should('eq', newUserData.image)
    UserSettingsPage.userPic().parent().should('have.text', newUserData.username)
    UserSettingsPage.getField(UserSettingsFields.bio).should('have.value', newUserData.bio)
    UserSettingsPage.getField(UserSettingsFields.email).should('have.value', newUserData.email)
    UserSettingsPage.getField(UserSettingsFields.password).invoke('val').should('eq', newUserData.password)
  })
})